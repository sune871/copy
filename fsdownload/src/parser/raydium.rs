...
pub fn parse_raydium_swap(
    ...
    // 解析指令数据获取交易金额
    let (amount_in, min_amount_out) = parse_swap_instruction_data(instruction_data)?;
    
    // 查找用户账户索引（通常是倒数第二个账户）
    let user_account_index = account_keys.len().saturating_sub(2);
    let user_wallet = Pubkey::from_str(&account_keys[user_account_index])
        .context("无法解析用户钱包地址")?;
    
    // 获取池子信息
    let pool_coin_account = &account_keys[5];
    let pool_pc_account = &account_keys[6];
    let user_source_account = &account_keys[15];
    let user_dest_account = &account_keys[16];
    ...
    // 计算gas费
    let gas_fee = calculate_gas_fee(pre_balances, post_balances, user_account_index);
    
    // 获取池子地址（AMM ID）
    let pool_address = Pubkey::from_str(&account_keys[1])
        .context("无法解析池子地址")?;
    ...
    let trade_details = TradeDetails {
        ...
        gas_fee,
        program_id: Pubkey::from_str(crate::types::RAYDIUM_AMM_V4)?,
    };
