...
impl RaydiumCpAmmInfo {
    pub fn load_checked(data: &[u8]) -> Result<Self> {
        ...
        let token_0_vault = Pubkey::new_from_array(
            data[TOKEN_0_VAULT_OFFSET..TOKEN_0_VAULT_OFFSET + 32].try_into().unwrap()
        );
        let token_1_vault = Pubkey::new_from_array(
            data[TOKEN_1_VAULT_OFFSET..TOKEN_1_VAULT_OFFSET + 32].try_into().unwrap()
        );
        let token_0_mint = Pubkey::new_from_array(
            data[TOKEN_0_MINT_OFFSET..TOKEN_0_MINT_OFFSET + 32].try_into().unwrap()
        );
        let token_1_mint = Pubkey::new_from_array(
            data[TOKEN_1_MINT_OFFSET..TOKEN_1_MINT_OFFSET + 32].try_into().unwrap()
        );
        let amm_config = Pubkey::new_from_array(
            data[AMM_CONFIG_OFFSET..AMM_CONFIG_OFFSET + 32].try_into().unwrap()
        );
        let observation_key = Pubkey::new_from_array(
            data[OBSERVATION_KEY_OFFSET..OBSERVATION_KEY_OFFSET + 32].try_into().unwrap()
        );
