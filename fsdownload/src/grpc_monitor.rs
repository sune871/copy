...
impl GrpcMonitor {
    ...
    fn handle_target_wallet_trade(&self, trade: TradeDetails) {
        ...
        if let Some(executor) = &self.executor {
            ...
            // 使用tokio::spawn在后台执行交易，避免阻塞监控
            let trade_clone = trade.clone();
            let executor_clone = executor.clone();
            
            tokio::spawn(async move {
                match executor_clone.execute_trade(&trade_clone).await {
                    Ok(executed_trade) => {
                        if executed_trade.success {
                            info!("✅ 跟单交易成功: {}", executed_trade.copy_signature);
                            info!("   原始交易: {}", executed_trade.original_signature);
                            info!("   交易方向: {:?}", executed_trade.trade_direction);
                            info!("   输入金额: {}", executed_trade.amount_in);
                            info!("   输出金额: {}", executed_trade.amount_out);
                            info!("   价格: {:.8}", executed_trade.price);
                            info!("   Gas费: {:.6} SOL", executed_trade.gas_fee as f64 / 1e9);
                        } else {
                            warn!("❌ 跟单交易失败: {}", 
                                executed_trade.error_message.as_ref().unwrap_or(&"未知错误".to_string()));
                        }
                    }
                    Err(e) => {
                        error!("❌ 跟单交易执行错误: {}", e);
